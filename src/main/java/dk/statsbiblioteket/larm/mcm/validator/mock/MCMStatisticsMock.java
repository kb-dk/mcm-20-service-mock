...
public class MCMStatisticsMock {
	...
	public String createStatusForObjectID(
        ...
        streamLogs.put(sessionID, streamLog);
		streamLog.add("StatsObject added with id: "+ sessionID);
        System.out.println("Returning: " + sessionID);
	    return sessionID;
	 }
	...
	public String setDurationSession( // endedAt=3917&startedAt=0
		...
		List<String> streamLog = streamLogs.get(sessionID);
		streamLog.add(sessionID + " - " + sessionID + "/" + objectSessionID + " [" + formatter.format(startedAt/1000.0) + "; " + formatter.format(endedAt/1000.0) + "]");
	...
	public String outputInternalStatus() {
		...
		for (List<String> streamLog : streamLogs.values()) {
			sb.append("  <session>\n");
			for (String logEntry : streamLog) {
